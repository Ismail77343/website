<?php
extract($_POST);


?>
<!DOCTYPE html>
<html lang="en">

<head>

    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="">
    <meta name="author" content="">

    <title>تسجيل دخولك</title>

    <!-- Custom fonts for this template-->
    <link href="<?php echo _Project."/Views/Admin/"; ?>vendor/fontawesome-free/css/all.min.css" rel="stylesheet" type="text/css">
    <link
        href="https://fonts.googleapis.com/css?family=Nunito:200,200i,300,300i,400,400i,600,600i,700,700i,800,800i,900,900i"
        rel="stylesheet">
        <link rel="icon" href="<?php echo _Project."/Views/Admin/img/1946429.png"; ?>" type="image/icon type">

    <!-- Custom styles for this template-->
    <link href="<?php echo _Project."/Views/Admin/"; ?>css/sb-admin-2.min.css" rel="stylesheet">
<style>
    *{
        text-align:right;
    }
</style>
</head>

<body class="bg-gradient-primary">

    <div class="container">

        <!-- Outer Row -->
        <div class="row justify-content-center">

            <div class="col-xl-10 col-lg-12 col-md-9">

                <div class="card o-hidden border-0 shadow-lg my-5">
                    <div class="card-body p-0">
                        <!-- Nested Row within Card Body -->
                        <div class="row">
                            <div class="col-lg-6 d-none d-lg-block bg-login-image"></div>
                            <div class="col-lg-6">
                                <div class="p-5">
                                    <div class="text-center">
                                        <h1 class="h4 text-gray-900 mb-4">مرحبا بك في لوحة التحكم!</h1>
                                        <?php
                                            echo $ctrl->check_msg_session("msg");
                                        ?>
                                    </div>
                                    <form class="user" action="<?php echo _Project."/Enter/"; ?>" method="post">
                                        <div class="form-group">
                                            <input type="text" name="UName" class="form-control form-control-user"
                                                id="exampleInputEmail" aria-describedby="emailHelp"
                                                placeholder="اسم المستخدم" autofocus>
                                        </div>
                                        <div class="form-group">
                                            <input type="password" class="form-control form-control-user"
                                                id="exampleInputPassword" name="Pass" placeholder="كلمة المرور">
                                        </div>
                                        
                                        <button type="submit" name="login" class="btn btn-primary btn-user btn-block">
                                            دخول
                                        </button>
                                        <hr>
                                    </form>                                   
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

            </div>

        </div>

    </div>

    <!-- Bootstrap core JavaScript-->
    <script src="<?php echo _Project."/Views/Admin/"; ?>vendor/jquery/jquery.min.js"></script>
    <script src="<?php echo _Project."/Views/Admin/"; ?>vendor/bootstrap/js/bootstrap.bundle.min.js"></script>

    <!-- Core plugin JavaScript-->
    <script src="<?php echo _Project."/Views/Admin/"; ?>vendor/jquery-easing/jquery.easing.min.js"></script>

    <!-- Custom scripts for all pages-->
    <script src="<?php echo _Project."/Views/Admin/"; ?>js/sb-admin-2.min.js"></script>

</body>

</html>